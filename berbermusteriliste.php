...
$toplam_icerik_sayisi = mysqli_num_rows(mysqli_query($conn,"select * from musteriler where berber='".$yetkikontrol["id"]."'"));
$limit = 20;
...
$cek	= mysqli_query($conn,"select * from musteriler  where berber='".$yetkikontrol["id"]."' LIMIT ".$baslangic.",".$limit."");
?>  
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
									<form method="post" action="yonetici.php?sayfa=berbermusteriarama" class="custom-validation">
...
if($toplam_icerik_sayisi > $limit){ 
	...
	if($sayfa > 1){	
		$onceki = $sayfa-1;	
			echo '  <li class="page-item">
                                                    <a class="page-link" href="yonetici.php?sayfa=berbermusteriliste&s='.$onceki.'" tabindex="-1">« Önceki</a></li>';
	...
	if($sayfa==1){ 
		...
		echo '<li class="page-item"><a class="page-link" href="yonetici.php?sayfa=berbermusteriliste&s=1">1</a></li>'; 	
	}
	...
	for($i; $i<=$sayfa+$x; $i++) { 
		if($i==$sayfa){ 
			...
			echo '<li class="page-item"><a class="page-link" href="yonetici.php?sayfa=berbermusteriliste&?s='.$i.'">'.$i.'</a></li>'; 
		}
		if($i==$sonSayfa) break;  
	}
	...
	if($sayfa+$x < $sonSayfa-1) { 
		echo '...'; 			
		echo '<li class="page-item"><a class="page-link" href="yonetici.php?sayfa=berbermusteriliste&s='.$sonSayfa.'">'.$sonSayfa.'</a></li>'; 
	...
	if($sayfa < $sonSayfa){  
		$sonraki = $sayfa+1; 	  
		echo '<li class="page-item"><a class="page-link" href="yonetici.php?sayfa=berbermusteriliste&s='.$sonraki.'"> Sonraki » </a></li>'; 
	}	
}
